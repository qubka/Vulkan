...
VkResult SwapChain::submitCommandBuffers(const VkCommandBuffer& buffers, const uint32_t& imageIndex) {
    ...
    VkFence& image = imagesInFlight[imageIndex];

    if (image != VK_NULL_HANDLE) {
        vkWaitForFences(device, 1, &image, VK_TRUE, UINT64_MAX);
    }
...
VkPresentModeKHR SwapChain::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) const {
    for (const auto &availablePresentMode : availablePresentModes) {
        if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) {
            //BOOST_LOG_TRIVIAL(trace) << "Present mode: Mailbox";
    ...
    //     //BOOST_LOG_TRIVIAL(trace) << "Present mode: Immediate";
    //     return availablePresentMode;
    //   }
    // }

    //BOOST_LOG_TRIVIAL(trace) << "Present mode: V-Sync";
