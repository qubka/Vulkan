...
static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
        ...
        void* pUserData) {
    //BOOST_LOG_TRIVIAL(error) << "validation layer: " << pCallbackData->pMessage;
    return VK_FALSE;
}
...
void Device::pickPhysicalDevice() {
    ...
    //BOOST_LOG_TRIVIAL(trace) << "Device count: " << deviceCount;
    ...
    //BOOST_LOG_TRIVIAL(trace) << "physical device: " << properties.deviceName;
}
...
void Device::hasGflwRequiredInstanceExtensions() const {
    ...
    //BOOST_LOG_TRIVIAL(trace) << "available extensions:";
    std::unordered_set<std::string> available;
    for (const auto& extension : extensions) {
        //BOOST_LOG_TRIVIAL(trace) << "\t" << extension.extensionName;
        available.insert(extension.extensionName);
    }

    //BOOST_LOG_TRIVIAL(trace) << "required extensions:";
    auto requiredExtensions = getRequiredExtensions();
    for (const auto& required : requiredExtensions) {
        //BOOST_LOG_TRIVIAL(trace) << "\t" << required;
